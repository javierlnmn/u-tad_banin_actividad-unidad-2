# Entrega 1 — Bitcoin Tweets Analysis (Kaggle Dataset)
...
> La **entrega 2** (análisis de textos con tópicos, sentimiento y resumen sobre tweets de la película de Michael Jackson vía RapidAPI) está descrita en [`README.md`](README.md).
...
| Opción | Descripción |
|--------|-------------|
| `--loader {csv,kaggle,json,rapidapi}` | Origen de datos. `json` no está implementado y termina con código de salida 2. |
...
## Dashboard interactivo (Kaggle / Bitcoin)

App **Streamlit** en la raíz del repo, sobre el mismo corpus de Bitcoin:

```bash
streamlit run dashboard_kaggle_tweets.py
```

Incluye ranking de hashtags, gráficos (Plotly), wordcloud y, en una pestaña, tópicos LDA, sentimiento y resumen extractivo sobre ese dataset.
